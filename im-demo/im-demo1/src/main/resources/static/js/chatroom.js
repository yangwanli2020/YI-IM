    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (data.code == 200) {
                ...
                var groupListHTML = "";
                var groupList = data.data.groupInfos;
                for (var i = 0; i < groupList.length; i++) {
                    var g = groupList[i];
                    roomIds.push(g.groupId)
                    if(g.groupType==1) {
                        groupListHTML +=
                            '<li>' +
                            '<div class="liLeft"><img src="' + g.groupAvatarUrl + '"></div>' +
                            '<div class="liRight">' +
                            '<span class="hidden-groupId">' + g.groupId + '</span>' +
                            '<span class="intername">' + g.groupName + '</span>' +
                            '<span class="infor"></span>' +
                            '</div>' +
                            '</li>';
                    }
                    if(g.groupType==2) {
                        var u = g.members[0].userId == userId ? g.members[1] : g.members[0];
                        groupListHTML +=
                            '<li>' +
                            '<div class="liLeft"><img src="' + u.avatarUrl + '"></div>' +
                            '<div class="liRight">' +
                            '<span class="hidden-groupId">' + g.groupId + '</span>' +
                            '<span class="hidden-userId">' + u.userId + '</span>' +
                            '<span class="intername">' + u.username + '</span>' +
                            '<span class="infor"></span>' +
                            '</div>' +
                            '</li>';
                    }
